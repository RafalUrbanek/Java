...
public class PlayerTab extends JPanel {
	...
	public PlayerTab() {
		...
		// statButtonPanel.setLayout(new BoxLayout(statButtonPanel, BoxLayout.Y_AXIS));
	...
	private static void addStatButons() {
		for (int i = 0; i < statButtonArray.length; i++) {
			...
			statButtonArray[i].setIcon(Support.scale(
					new ImageIcon("src\\resources\\skillPoint.png"), 26, 16));
	...
	private static void addPotion() {
		invButton[0].setIcon(HealthSmall
				.scale(new ImageIcon("src\\resources\\healthPotion2.png")));
	...
	public static boolean addItem(String ID) {
		...
		if (!possesedItems.contains(ID)) {
			...
			if (nextEmpty != -1) {
				invButton[nextEmpty].setIcon(Support.scale(new ImageIcon(item.url), invButton[nextEmpty].getWidth(),
						invButton[nextEmpty].getHeight()));
